...
# Helper function to run external commands and verify they succeeded
function Invoke-And-Check
{
    & $args
    if ($LASTEXITCODE -ne 0)
    {
        # This will trigger the $ErrorActionPreference and stop the script
        throw "Command '$( $args[0] )' failed with exit code $LASTEXITCODE."
    }
}
